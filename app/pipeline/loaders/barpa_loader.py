...
class BARPALoader(BaseTCLoader):
    ...
    def load(self) -> list:
        ...
        with self.source_file.open('r', encoding='utf-8-sig', newline='') as f:
            ...
            for row in reader:
                track_id = row.get('track_id') or row.get('storm_id') or row.get('tc_id')
                if not track_id:
                    continue
                grouped[track_id].append(
                    TCPoint(
                        ...
                        lat=float(row['lat']),
                        lon=float(row['lon']),
                        wind_speed=self.to_float(row.get('wind_speed') or row.get('wind')),
                        pressure=self.to_float(row.get('pressure')),
                        category=self.to_int(row.get('category')),
                        over_land=(row.get('over_land', '').strip().lower() in {'1', 'true', 'yes'}) if row.get('over_land') is not None else None,
                    )
                )
        return [

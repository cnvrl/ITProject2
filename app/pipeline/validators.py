...
class ValidationReport:
    ...
    def add(
        ...
        message: str,
        record_id: Optional[str] = None,
        point_index: Optional[int] = None,) -> None:
        self.issues.append(
            ValidationIssue(
                severity=severity,
                code=code,
...
def validate_point(point: TrackPoint, record_id: str, point_index: int,) -> ValidationReport:
    ...
    if (
        point.category is not None
        and not 0 <= point.category <= 5
    ):
        report.add(
            "error",
            "invalid_category",
    ...
    if (
        point.pressure is not None
        and point.pressure <= 0
    ):
        report.add(
            "warning",
            "invalid_pressure",
...
def validate_record(record: TCRecord,) -> ValidationReport:
    ...
    if not str(record.dataset_id).strip():
        report.add(
            "error",
            "missing_dataset_id",
            "The record has no dataset identifier.",
            record_id,
        )
    ...
    if not str(record.track_id).strip():
        report.add(
            "error",
            "missing_track_id",
            "The record has no track identifier.",
            record_id,
        )
    ...
    if not str(record.model).strip():
        report.add(
            "error",
            "missing_dataset",
            "The record has no BARPA/CCAM dataset value.",
            record_id,
        )
    ...
    if not str(record.source_model).strip():
        report.add(
            "error",
            "missing_driving_model",
            "The record has no driving-model value.",
            record_id,
        )
    ...
    if not str(record.tracker).strip():
        report.add(
            "error",
            "missing_tracker",
            "The record has no tracker value.",
            record_id,
        )
    ...
    if record.season is None and record.year is None:
        report.add(
            "error",
            "missing_season",
            "The record has no season or year.",
            record_id,
        )
    ...
    if not record.points:
        report.add(
            "error",
            "missing_points",
            "The cyclone track contains no observations.",
            record_id,
        )
    ...
    if (
        ...
        report.add(
            "error",
            "invalid_max_category",
            "Maximum category must be between 0 and 5.",
            record_id,
        )

    if (
        record.max_wind_speed is not None
        and record.max_wind_speed < 0
    ):
        report.add(
            "error",
            "invalid_max_wind",
            "Maximum wind speed cannot be negative.",
            record_id,
        )

    if (
        record.lifetime_hours is not None
        and record.lifetime_hours < 0
    ):
        report.add(
            "error",
            "invalid_lifetime",
            "Cyclone lifetime cannot be negative.",
            record_id,
        )
    ...
    for record in records:
        ...
        if record.identity in seen_identities:
            report.add(
                ...
                (
                    "Another record has the same dataset, "
                    "model, tracker, season, and raw track ID."
                ),
                record.track_id,
            )
